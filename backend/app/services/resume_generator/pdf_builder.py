...
def build_resume_html(
    ...
    for exp in experience:
        # ── Null guards — Python None must never reach the HTML ──────
        ...
        location_text = f" • {loc}" if loc else ""

        # Date range — only show if we have at least one date
        if start or end != "Present":
            date_range = f"{start}{' - ' + end if end else ''}"
        else:
            date_range = ""
        ...
        <div class="experience-item">
            ...
            {f'<div class="date-location">{date_range}{location_text}</div>' if date_range or location_text else ''}
            {desc_html}
            {achievements_html}
            {tech_html}
        </div>
    ...
    for edu in education:
        ...
        grad_date = edu.get("graduation_date") or ""  # ← was printing "None"
        ...
        <div class="education-item">
            ...
            {f'<div class="company">{school}</div>' if school else ''}
            {f'<div class="date-location">{grad_date}{location_text}{gpa_text}</div>' if any([grad_date, loc, gpa]) else ''}
        </div>
        """
...
<head>
    ...
    <style>
        ...
        h1 {{ margin: 0; font-size: 32px; color: #1f2937; }}
        .contact {{ margin-top: 10px; font-size: 14px; color: #6b7280; }}
        ...
        .summary {{ font-size: 15px; line-height: 1.8; color: #4b5563; }}
        ...
        .skill.matched {{ background: #dcfce7; color: #16a34a; }}
        .experience-item, .education-item {{ margin-bottom: 20px; }}
        .job-title {{ font-size: 16px; font-weight: 600; color: #1f2937; margin-bottom: 3px; }}
        .company {{ font-size: 15px; color: #2563eb; margin-bottom: 3px; }}
        .date-location {{ font-size: 13px; color: #6b7280; margin-bottom: 8px; }}
        .description {{ font-size: 14px; color: #4b5563; margin-bottom: 8px; }}
        ul {{ margin: 0; padding-left: 20px; }}
        li {{ font-size: 14px; color: #4b5563; margin-bottom: 4px; }}
